 ...
 */
function parseQuakeML(quakeML: string): EarthquakeEpicenter[] {
  const parser = new DOMParser();
  const xmlDoc = parser.parseFromString(quakeML, "application/xml");

  const nsResolver: XPathNSResolver = (prefix: string | null) => {
    const ns: { [key: string]: string } = { q: "http://quakeml.org/xmlns/bed/1.2" };
    return prefix ? ns[prefix] : null;
  };

  const events = xmlDoc.evaluate(
    "//q:event",
    xmlDoc,
    nsResolver,
  ...
  for (let i = 0; i < events.snapshotLength; i++) {
    ...
    const magnitudeElement = xmlDoc.evaluate(
      ".//q:magnitude",
      event,
      nsResolver,
      XPathResult.FIRST_ORDERED_NODE_TYPE,
    ...
    const descriptionElement = xmlDoc.evaluate(
      ".//q:description",
      event,
      nsResolver,
      XPathResult.FIRST_ORDERED_NODE_TYPE,
    ...
    const depthElement = xmlDoc.evaluate(
      ".//q:depth/q:value",
      origin,
      nsResolver,
      XPathResult.FIRST_ORDERED_NODE_TYPE,
    ...
    const timeElement = xmlDoc.evaluate(
      ".//q:time/q:value",
      origin,
      nsResolver,
      XPathResult.FIRST_ORDERED_NODE_TYPE,
    ...
    if (origin && magnitudeElement && descriptionElement) {
      const latitudeElement = xmlDoc.evaluate(
        ".//q:latitude/q:value",
        origin,
        nsResolver,
        XPathResult.FIRST_ORDERED_NODE_TYPE,
      ...
      const longitudeElement = xmlDoc.evaluate(
        ".//q:longitude/q:value",
        origin,
        nsResolver,
        XPathResult.FIRST_ORDERED_NODE_TYPE,
      ...
      const magnitudeValueElement = xmlDoc.evaluate(
        ".//q:mag/q:value",
        magnitudeElement,
        nsResolver,
        XPathResult.FIRST_ORDERED_NODE_TYPE,
      ...
      const placeElement = xmlDoc.evaluate(
        ".//q:text",
        descriptionElement,
        nsResolver,
        XPathResult.FIRST_ORDERED_NODE_TYPE,
 ...
 */
function toggleMagnitudeInputs(): void {
  const disableMagnitudeFilter = (
    document.getElementById("disableMagnitudeFilter") as HTMLInputElement
  ).checked;
  const minMagnitudeInput = document.getElementById("minMagnitude") as HTMLInputElement;
  const maxMagnitudeInput = document.getElementById("maxMagnitude") as HTMLInputElement;
 ...
 */
function getSelectedTimeRange(): string {
  let timeRange = "";
 ...
 */
function getSelectedContinent(): string {
  const continent = (document.getElementById("continentFilter") as HTMLSelectElement).value;
  return continent;
}

document.getElementById("submit")!.addEventListener("click", () => {
  const minMagnitude = parseFloat((document.getElementById("minMagnitude") as HTMLInputElement).value);
  const maxMagnitude = parseFloat((document.getElementById("maxMagnitude") as HTMLInputElement).value);
 ...
 */
function updateEarthquakeData(epicenter: EarthquakeEpicenter): void {
